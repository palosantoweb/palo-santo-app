  ...
  return (
    <div className='flex items-center justify-between bg-gray-900 border-t-[1.5px]  border-gray-500 text-white relative bottom-0'>
      <div className='mt-0 mb-3 flex'>
        <Image src={PS} alt='PaloSanto' />
      </div>
      <div className='flex flex-col items-end'>
        {links.map(({ label, route }) => {
          return (
            <div key={route} className='flex items-center ml-6 mr-6'>
              <Link href={route}>
                <span className='text-[#CC8942]'>{label}</span>
              </Link>
            </div>
          );
        })}
        {session?.user ? (
          ...
          <div className='flex items-end ml-6 mr-6'>
            <a
              href='/api/auth/login'
              className='text-[#CC8942] text-sm whitespace-nowrap'
            >
              Soy administrador
            </a>
          </div>
