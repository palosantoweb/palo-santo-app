...
let basePath = path.join(__dirname, './images')
...
async function read(fileName: string, dirPrefix: string) {
    ...
    try {
        ...
        throw new RequestError(`Ocurrio un error al leer la imagen <${fileName}}>.`, 500)
    }
}
...
async function write(file: FileModel, dirPrefix: string) {
    ...
    try {
        ...
        throw new RequestError(`Ocurrio un error al guardar la imagen <${file.name}}>.`, 500)
    }
}

export async function remove(fileName: string, dirPrefix: string) {
    const dir = basePath + dirPrefix
    try {
        await promisify(unlink)(`${dir}/${fileName}`);
    } catch (error) {
        throw new RequestError(`Ocurrio un error al eliminar la imagen <${fileName}}>.`, 500)
    }
}